...
export function Login() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await getMe();
      router.push(routes.admin.dashboard.getHref());
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min flex h-screen items-center justify-center bg-gray-100">
      <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-md">
        <h2 className="text-center text-2xl font-semibold text-[#667467]">
          Lucie Perrier
        </h2>
        ...
        <form onSubmit={onSubmit}>
          ...
          <button
            type="submit"
            className="w-full cursor-pointer rounded bg-[#667467] px-4 py-2 font-semibold text-white hover:bg-[#445a46]"
          >
            Connexion
          </button>
        </form>
